...
class Docs(BaseModel, arbitrary_types_allowed=True, smart_union=True):
    ...
    def add(
        ...
        # 增加摘要
        doc = Doc(docname=docname, citation=citation, dockey=dockey,abstract =abstract)
    ...
    async def adoc_match(
        ...
        if self.doc_index is None:
            if len(self.docs) == 0:
                return set()
            # 从文档中获取文本列表和元数据列表
            abstract = [doc.abstract for doc in self.docs.values()]
            metadatas = [d.dict() for d in self.docs.values()]
            # 使用文本和元数据构建FAISS向量库索引
            self.doc_index = FAISS.from_texts(
                abstract, metadatas=metadatas, embedding=self.embeddings
            )
        
        # 重写并且翻译问题
        complete_chain = make_chain(
                prompt=self.prompts.completation,
                llm=cast(BaseLanguageModel, self.summary_llm),
                skip_system=True,
            )
        query_completed = complete_chain.run(query)
        
        # 在文档索引中使用最大边际相关性搜索查询匹配项
        matches = self.doc_index.max_marginal_relevance_search(
            query_completed, k=k + len(self.deleted_dockeys)
        )
        # filter the matches
        # 过滤掉已删除的文档键
        matches = [
    ...
    async def aget_evidence(
        ...
        context_str = "\n\n".join(
            [
                f"{c.text.doc.citation}: {c.context}"
                + (f"\n\n Based on {c.text.name}" if detailed_citations else "")
    ...
    async def aquery(
        ...
        for c in answer.contexts:
            ...
            # if name_in_text(citation, answer_text):
            bib[name] = citation
                
        bib_str = "\n\n".join(
            [f"{i+1}. {c}:({k}) " for i, (k, c) in enumerate(bib.items())]
